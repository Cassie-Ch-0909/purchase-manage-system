...
import { ref } from "vue";
import { getCollectOrderListAPI, cancelAPI } from "@/api/order";
...
async function getCollectOrderList() {
  const res = await getCollectOrderListAPI();
  console.log(res);
...
const handleEdit = (index, row) => {
  ElMessageBox.confirm("此操作将撤销订单该文件, 是否继续?", "提示", {
    ...
    .then(async () => {
    //   console.log("--------------------------");
      let res = await cancelAPI({ id: row.id });
